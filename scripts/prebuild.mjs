...
function generateToolLastmod() {
  ...
  try {
    ...
    for (const token of raw.split("\0")) {
      ...
      for (const line of trimmedToken.split("\n")) {
        ...
        // Skip known non-tool helpers
        if (["trust-badges", "share-buttons", "example-url-listener", "finance-disclaimer", "tool-interface"].includes(base)) continue;
  ...
  // Intersect with current registry slugs so renames/deleted helpers don't leak stale keys
  let registrySlugs = new Set();
  try {
    const toolsTs = readFileSync(join(ROOT, "src", "lib", "data", "tools.ts"), "utf-8");
    for (const m of toolsTs.matchAll(/slug:\s*"([^"]+)"/g)) registrySlugs.add(m[1]);
  } catch {}
  const filtered = registrySlugs.size > 0 ? Object.fromEntries(Object.entries(lastmod).filter(([k]) => registrySlugs.has(k))) : lastmod;
  const sorted = Object.fromEntries(Object.entries(filtered).sort(([a], [b]) => (a < b ? -1 : 1)));
