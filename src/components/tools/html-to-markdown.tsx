...
export function HtmlToMarkdown() {
  ...
  const [hrStyle, setHrStyle] = useState<HrStyle>("---");
  ...
  const convert = useCallback(() => {
    const opts = { headingStyle, bulletMarker, linkStyle, hrStyle, codeStyle, strongEmStyle, stripStyles, preserveAlt };
  ...
  }, [input, headingStyle, bulletMarker, linkStyle, hrStyle, codeStyle, strongEmStyle, stripStyles, preserveAlt]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-xs text-surface-500 dark:text-dark-muted">HR:</label>
          <select value={hrStyle} onChange={(e) => setHrStyle(e.target.value as HrStyle)}
            className="rounded border border-surface-200 bg-white px-2 py-1 text-xs text-surface-700 dark:border-dark-border dark:bg-dark-surface dark:text-dark-text">
            <option value="---">---</option>
            <option value="***">***</option>
      ...
      <div className="flex gap-2">
        <button onClick={copy} disabled={!output} className="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-40 transition-colors">Copy Markdown</button>
        <button onClick={download} disabled={!output} className="rounded-lg border border-surface-200 px-4 py-2 text-sm font-medium text-surface-700 hover:bg-surface-50 disabled:opacity-40 dark:border-dark-border dark:text-dark-text dark:hover:bg-dark-surface transition-colors">Download .md</button>
