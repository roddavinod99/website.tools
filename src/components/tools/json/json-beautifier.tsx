...
const themes: Record<string, { bg: string; text: string; key: string; string: string; number: string; boolean: string; null: string; bracket: string }> = {
  ...
  github: { bg: "bg-surface-50 dark:bg-dark-bg", text: "text-surface-800 dark:text-dark-text", key: "text-[#0550AE] dark:text-brand-300", string: "text-[#0A3069] dark:text-brand-200", number: "text-[#0550AE] dark:text-brand-300", boolean: "text-[#CF222E] dark:text-[#FF7B72]", null: "text-surface-500 dark:text-dark-muted", bracket: "text-surface-800 dark:text-dark-text" },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={beautify} className="rounded-md bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 transition-colors">Beautify</button>
        <button onClick={copy} disabled={!output} className="rounded-md border border-surface-200 px-4 py-2 text-sm font-medium text-surface-700 hover:bg-surface-50 disabled:opacity-40 dark:border-dark-border dark:text-dark-text dark:hover:bg-dark-surface transition-colors">Copy</button>
        <button onClick={download} disabled={!output} className="rounded-md border border-surface-200 px-4 py-2 text-sm font-medium text-surface-700 hover:bg-surface-50 disabled:opacity-40 dark:border-dark-border dark:text-dark-text dark:hover:bg-dark-surface transition-colors">Download</button>
        <button onClick={print} disabled={!output} className="rounded-md border border-surface-200 px-4 py-2 text-sm font-medium text-surface-700 hover:bg-surface-50 disabled:opacity-40 dark:border-dark-border dark:text-dark-text dark:hover:bg-dark-surface transition-colors">Print</button>
        <button onClick={() => setShowDiff(!showDiff)} disabled={!input || !output} className={`rounded-md border px-4 py-2 text-sm font-medium transition-colors disabled:opacity-40 ${showDiff ? "bg-brand-50 border-brand-300 text-brand-700 dark:bg-brand-900/20 dark:border-brand-700 dark:text-brand-400" : "border-surface-200 text-surface-700 hover:bg-surface-50 dark:border-dark-border dark:text-dark-text dark:hover:bg-dark-surface"}`}>
      ...
      {showDiff && formattedOrigin && output && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <pre className={`rounded-md border border-surface-200 p-3 text-xs font-mono overflow-auto max-h-64 ${theme.bg} ${theme.text}`}>
      ...
      {!showDiff && output && (
        <div>
          ...
          <pre className={`w-full rounded-md border border-surface-200 p-3 text-sm font-mono overflow-auto max-h-96 ${theme.bg} ${theme.text}`}>
            {highlighted}
          </pre>
        </div>
      )}
