  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div><label className="block text-xs text-surface-500 dark:text-dark-muted mb-1">Draft</label><select value={draft} onChange={(e) => setDraft(e.target.value as "07" | "2020-12")} className="rounded-md border border-surface-200 bg-white px-3 py-2 text-sm dark:border-dark-border dark:bg-dark-surface dark:text-dark-text"><option value="07">Draft-07</option><option value="2020-12">Draft-2020-12</option></select></div>
      ...
      <div className="flex flex-wrap gap-1">
        <button onClick={() => handleCopy(output, "schema")} disabled={!output} className="rounded bg-brand-500 px-2 py-1 text-xs text-white hover:bg-brand-600 disabled:opacity-40" aria-label="Copy schema">{copied === "schema" ? "Copied!" : "Copy"}</button>
        <button onClick={download} disabled={!output} className="rounded border border-surface-200 bg-white px-2 py-1 text-xs text-surface-700 hover:bg-surface-50 disabled:opacity-40 dark:border-dark-border dark:bg-dark-surface dark:text-dark-text" aria-label="Download schema">Download .json</button>
      </div>
