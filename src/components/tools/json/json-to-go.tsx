  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-1">
        <button onClick={handleCopy} disabled={!output} className="rounded bg-brand-500 px-2 py-1 text-xs text-white hover:bg-brand-600 disabled:opacity-40" aria-label="Copy Go code">{copied ? "Copied!" : "Copy"}</button>
        <button onClick={download} disabled={!output} className="rounded border border-surface-200 bg-white px-2 py-1 text-xs text-surface-700 hover:bg-surface-50 disabled:opacity-40 dark:border-dark-border dark:bg-dark-surface dark:text-dark-text" aria-label="Download Go code">Download .go</button>
      </div>
