...
import { Copy, RefreshCw, Minimize } from "lucide-react";

type KeywordCase = "upper" | "lower" | "capitalize";
type Indent = "2" | "4" | "tab";
type CommaPos = "before" | "after";
type Dialect = "generic" | "mysql" | "postgresql" | "sqlserver" | "oracle" | "sqlite" | "mariadb" | "db2" | "bigquery" | "redshift" | "snowflake";
...
export function SQLFormatter() {
  ...
  }, [input, keywordCase, indentW, lineWidth, commaPos, newLineDistinct, logicalNewline, trailingCommas]);
  ...
  const dialectSpecifics: Record<Dialect, string> = {
    generic: "", mysql: "/* MySQL dialect */", postgresql: "/* PostgreSQL dialect */",
    sqlserver: "/* SQL Server dialect */", oracle: "/* Oracle dialect */", sqlite: "/* SQLite dialect */",
    mariadb: "/* MariaDB dialect */", db2: "/* DB2 dialect */", bigquery: "/* BigQuery dialect */",
    redshift: "/* Redshift dialect */", snowflake: "/* Snowflake dialect */",
  };
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
        <div>
          ...
          <select value={dialect} onChange={(e) => setDialect(e.target.value as Dialect)}
            ...
            <option value="generic">Generic</option><option value="mysql">MySQL</option><option value="postgresql">PostgreSQL</option>
            <option value="sqlserver">SQL Server</option><option value="oracle">Oracle</option><option value="sqlite">SQLite</option>
            <option value="mariadb">MariaDB</option><option value="db2">DB2</option><option value="bigquery">BigQuery</option>
            <option value="redshift">Redshift</option><option value="snowflake">Snowflake</option>
          </select>
        </div>
        <div>
          ...
          <select value={keywordCase} onChange={(e) => setKeywordCase(e.target.value as KeywordCase)}
            className="w-full rounded border border-surface-200 bg-white px-2 py-1 text-xs text-surface-700 dark:border-dark-border dark:bg-dark-surface dark:text-dark-text">
            <option value="upper">UPPER</option><option value="lower">lower</option><option value="capitalize">Capitalize</option>
          </select>
        </div>
        <div>
      ...
      {output && (
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs text-surface-400 dark:text-dark-muted">{dialectSpecifics[dialect]}</span>
          </div>
