...
function rgbToCmyk({ r, g, b }: RGB): CMYK {
  ...
  const c = ((1 - mr - k) / (1 - k)) * 100;
  const m = ((1 - mg - k) / (1 - k)) * 100;
  const y = ((1 - mb - k) / (1 - k)) * 100;
...
function generatePalette(base: RGB): { label: string; hex: string }[] {
  ...
  for (let i = 0; i <= 5; i++) {
    const f = 0.2 + i * 0.15;
    const nr = Math.min(255, Math.round(r + (255 - r) * (1 - f)));
    const ng = Math.min(255, Math.round(g + (255 - g) * (1 - f)));
    const nb = Math.min(255, Math.round(b + (255 - b) * (1 - f)));
    palette.push({ label: `Tint ${i + 1}`, hex: rgbToHex({ r: nr, g: ng, b: nb }) });
  }
  for (let i = 0; i <= 4; i++) {
    const f = 0.2 + i * 0.15;
    ...
    palette.push({ label: `Shade ${i + 1}`, hex: rgbToHex({ r: nr, g: ng, b: nb }) });
  }
  return palette;
}
...
export function ColorConverter() {
  ...
  const hexShort = useMemo(() => {
    if (!rgb) return "";
    const { r, g, b } = rgb;
    if (r % 17 === 0 && g % 17 === 0 && b % 17 === 0) {
      return "#" + [r, g, b].map((c) => (c / 17).toString(16)).join("");
    }
    return "";
  }, [rgb]);
  ...
  const cssName = useMemo(() => {
    ...
    for (const [name, code] of Object.entries(namedColors)) {
      if (code.toUpperCase() === hexUpper) return name;
    }
    return "";
  }, [hex, rgb]);
  ...
  const colorDiff = useMemo(() => {
    if (!rgb || !fgRgb) return null;
    return deltaE(rgb, fgRgb);
  }, [rgb, fgRgb]);
  ...
  const items: { label: string; value: string; key: string }[] = [
    ...
    { label: "HEX (8-digit with alpha)", value: rgb && opacity < 1 ? hex.replace(/^.{7}/, (m) => m.slice(0, 6) + Math.round(opacity * 255).toString(16).padStart(2, "0")) : hex, key: "hex8" },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div
          className="h-12 w-20 rounded-lg border border-surface-200 dark:border-dark-border shrink-0"
        ...
        <div className="flex-1">
          ...
          <input
            ...
            placeholder="#000000, rgb(r,g,b), hsl(h,s,l), crimson..."
      ...
      <div>
        <label className="block text-sm font-medium text-surface-700 dark:text-dark-text mb-1">
          Opacity / Alpha: {opacity.toFixed(2)}
        </label>
        <input
          type="range"
          min={0}
      ...
      {!rgb && (
        <p className="text-sm text-red-500">Invalid color format. Try HEX, RGB, HSL, or a named color.</p>
      )}
      ...
      {rgb && (
        <div className="space-y-2">
          {items.map((item) => (
            ...
            >
              <span className="text-sm text-surface-500 dark:text-dark-muted w-28 shrink-0">{item.label}</span>
        ...
        <>
          <div className="border-t border-surface-200 pt-4 dark:border-dark-border">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div>
                <p className="text-xs text-surface-500 dark:text-dark-muted mb-1">Shades & Tints</p>
                <div className="flex gap-1">
                  {palette.slice(0, 6).map((c, i) => (
                    <button
                      key={`t${i}`}
                      className="h-6 w-6 rounded border border-surface-200 dark:border-dark-border cursor-pointer"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {presets.map((c, i) => (
                    <button
                      key={i}
                      className="h-5 w-5 rounded border border-surface-200 dark:border-dark-border cursor-pointer"
                      style={{ backgroundColor: c }}
                      onClick={() => setInput(c)}
                      title={c}
                    />
          ...
          {colorDiff !== null && (
            <p className="text-xs text-surface-400 dark:text-dark-muted">
              Color Difference (Delta E): {colorDiff}
            </p>
          )}
        </>
      )}
