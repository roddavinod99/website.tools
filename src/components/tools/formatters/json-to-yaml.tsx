    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <select value={direction} onChange={(e) => { setDirection(e.target.value as Direction); setInput(""); setOutput(""); setError(""); }}
          className="rounded-md border border-surface-200 bg-white px-3 py-1.5 text-sm text-surface-700 dark:border-dark-border dark:bg-dark-surface dark:text-dark-text">
      ...
      <div className="flex gap-2">
        <button onClick={copy} disabled={!output} className="rounded-md bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-40 transition-colors">Copy {direction === "json-to-yaml" ? "YAML" : "JSON"}</button>
