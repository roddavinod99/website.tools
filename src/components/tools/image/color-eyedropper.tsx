...
export function ColorEyedropper() {
  ...
  const pickColor = async () => {
    if (!eyeDropperSupported) {
      setEyeDropperError("EyeDropper API is not supported in this browser. Try Chrome 95+, Edge 95+, or Opera 81+.");
      setTimeout(() => setEyeDropperError(""), 3000);
      return;
    }
    try {
      ...
      setEyeDropperError("Color picking was cancelled or failed.");
      setTimeout(() => setEyeDropperError(""), 3000);
    }
  };
  ...
  const inputCls = "w-full rounded-md border border-surface-200 bg-white px-3 py-2 text-sm font-mono text-surface-900 placeholder:text-surface-400 focus:outline-none focus:ring-2 focus:ring-brand-400 dark:border-dark-border dark:bg-dark-surface dark:text-dark-text";

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="h-12 w-20 rounded-md border border-surface-200 dark:border-dark-border shrink-0" style={{ backgroundColor: hex }} />
        ...
        <button onClick={pickColor}
          className="rounded-md bg-brand-500 px-3 py-2 text-sm font-medium text-white hover:bg-brand-600 transition-colors flex items-center gap-1"
          title={eyeDropperSupported ? "Pick color from screen" : "Not supported in this browser"}>
          Pick Color
        </button>
      </div>

      {eyeDropperError && (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400">{eyeDropperError}</div>
      )}
      ...
      <div className="space-y-1.5">
        {[["HEX", hex], ["RGB", rgb ? `rgb(${rgb.r}, ${rgb.g}, ${rgb.b})` : ""], ["HSL", hsl ? `hsl(${hsl.h}, ${hsl.s}%, ${hsl.l}%)` : ""], ["HSV", hsv ? `hsv(${hsv.h}, ${hsv.s}%, ${hsv.v}%)` : ""], ["CMYK", cmyk ? `cmyk(${cmyk.c}%, ${cmyk.m}%, ${cmyk.y}%, ${cmyk.k}%)` : ""]].map(([label, value]) => (
          <div key={label} className="flex items-center justify-between rounded-md border border-surface-200 bg-surface-50 p-2 dark:border-dark-border dark:bg-dark-surface">
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={saveToPalette} className="rounded-md border border-surface-200 px-3 py-1.5 text-xs font-medium text-surface-700 hover:bg-surface-50 dark:border-dark-border dark:text-dark-text">Save to Palette</button>
        <button onClick={() => exportPalette("css")} className="rounded-md border border-surface-200 px-3 py-1.5 text-xs font-medium text-surface-700 hover:bg-surface-50 dark:border-dark-border dark:text-dark-text">Export CSS</button>
        <button onClick={() => exportPalette("json")} className="rounded-md border border-surface-200 px-3 py-1.5 text-xs font-medium text-surface-700 hover:bg-surface-50 dark:border-dark-border dark:text-dark-text">Export JSON</button>
        <button onClick={() => exportPalette("tailwind")} className="rounded-md border border-surface-200 px-3 py-1.5 text-xs font-medium text-surface-700 hover:bg-surface-50 dark:border-dark-border dark:text-dark-text">Export Tailwind</button>
      </div>

      {history.length > 0 && (
        <div>
          <p className="text-xs font-medium text-surface-700 dark:text-dark-text mb-1">History (last 20)</p>
