...
export function NumeronymGenerator() {
  ...
  return (
    <div className="space-y-4">
      ...
      {numeronyms.length > 0 && (
        <>
          <button onClick={copyAll} className="rounded-md bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 transition-colors">
            {copied ? "Copied!" : "Copy All"}
          </button>

          <div className="space-y-1.5">
            {numeronyms.map((n, i) => (
              <div key={i} className="flex items-center gap-3 p-2.5 rounded-md border border-surface-200 dark:border-dark-border bg-white dark:bg-dark-surface group">
