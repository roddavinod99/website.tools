  ...
  return (
    <div className="space-y-4">
      ...
      <AdvancedOptions title="Advanced options" defaultOpen={false}>
        <OptionGroup title="Match Options" description="Control how matches are found and displayed">
          <OptionRow columns={3}>
            ...
            <label className="flex items-center gap-2 text-sm text-surface-700 dark:text-dark-text">
              <input type="checkbox" checked={showCheatSheet} onChange={(e) => setShowCheatSheet(e.target.checked)} className="accent-brand-500" />
              Show cheat sheet
            </label>
            <label className="flex items-center gap-2 text-sm text-surface-700 dark:text-dark-text">
              <input type="checkbox" checked={showLibrary} onChange={(e) => setShowLibrary(e.target.checked)} className="accent-brand-500" />
              Show pattern library
            </label>
          </OptionRow>
        </OptionGroup>

        <OptionGroup title="Match Details" description="Configure what information is shown for each match">
          <OptionRow columns={2}>
            <label className="flex items-center gap-2 text-sm text-surface-700 dark:text-dark-text">
              <input type="checkbox" checked={showHistory} onChange={(e) => setShowHistory(e.target.checked)} className="accent-brand-500" />
              Show pattern history
            </label>
          </OptionRow>
        </OptionGroup>

        <OptionGroup title="Export & Copy" description="Export match results in various formats">
          <OptionRow columns={3}>
            ...
            <button onClick={() => copy(JSON.stringify(matches, null, 2), 1)} className="rounded px-2 py-0.5 text-xs border border-surface-200 text-surface-600 hover:bg-surface-50 dark:border-dark-border dark:text-dark-muted dark:hover:bg-dark-surface">Copy Matches (JSON)</button>
            <button onClick={exportResults} className="rounded px-2 py-0.5 text-xs border border-surface-200 text-surface-600 hover:bg-surface-50 dark:border-dark-border dark:text-dark-muted dark:hover:bg-dark-surface">Export All</button>
          </OptionRow>
        </OptionGroup>
      </AdvancedOptions>
