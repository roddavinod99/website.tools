      ...
      {output.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <button onClick={copy} className="rounded-md bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600">Copy</button>
          </div>
          <pre className="w-full rounded-md border border-surface-200 bg-surface-50 p-3 text-sm font-mono dark:border-dark-border dark:bg-dark-bg dark:text-dark-text overflow-auto max-h-60 select-all whitespace-pre-wrap">{output.join("\n")}</pre>
        </div>
      )}
