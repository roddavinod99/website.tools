...
import { getPopularCurrencies, getCurrency } from "@/lib/data/currencies";
...
export function CurrencySelector({
  ...
  const displayValue = selectedCurrency
    ? `${selectedCurrency.code} \u2014 ${selectedCurrency.name} (${selectedCurrency.symbolNative})`
    : placeholder;
          ...
          >
            {filtered.map((currency, index) => (
              <li
                ...
                aria-label={`${currency.code} \u2014 ${currency.name} (${currency.symbolNative})`}
                onClick={() => handleOptionClick(currency.code)}
                onMouseEnter={() => setHighlightedIndex(index)}
                className={cn(
                  "px-3 py-2 text-sm cursor-pointer transition-colors",
                  index === highlightedIndex && "bg-brand-50 dark:bg-brand-900/20",
                  currency.code === value && "font-medium text-brand-600 dark:text-brand-400"
                )}
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono">{currency.code}</span>
                  <span className="text-surface-500 dark:text-dark-muted ml-2">{currency.symbolNative}</span>
                </div>
                <div className="text-xs text-surface-500 dark:text-dark-muted truncate">{currency.name}</div>
              </li>
            ))}
            {filtered.length === 0 && (
